...
const TeacherList: React.FC = () =>  {  
  ...
  const deleteHandler = (id: string) => {
       axios.delete(`http://localhost:3005/teachers/${id}`);
            ...
            {data && (
              <div className='flex'>
                <div className='w-full'>
                  <h1 className=' flex  justify-center items-center text-xl text-black font-bold mt-4'>Teacher List</h1>  
                  {data.map((option: any) => (
                    <div key={uniqueID()} className='flex flex-col justify-center items-center'> 
                      ...
                      <Link href={`/Components/studentEdit/${option.id}`} className="bg-green-600 hover:bg-green-700 text-white font-semibold py-[5px] px-[5px] ml-3 transition rounded-full duration-200 ease-in-out transform text-sm">
                      ...
                      <Modal open={open} onClose={() => setOpen(false)}>
                        <div className="text-center w-56">
                          ...
                          <div className="flex gap-4">
                            <button onClick={() => deleteHandler(option.id)} className="btn bg-red-600 hover:bg-red-700 rounded-md w-full">Delete</button>
                            <button
