...
const TeacherList = () =>  {  
  ...
  const deleteHandler = (id:any) =>{
    ...
    const newList = data.filter ( (data:any) => data.id !== id)
    console.log(data)
    axios.delete(`https://crudcrud.com/api/352c3afb135a4ea7abe461f02981c8e1/teacher/${newList}`)
    router.push('/');
  }
                ...
                <div className='w-full'>
                  {data.map((option: any) => (
                    <div key={option._id}>
                      ...
                      <Link href={{ 
                          pathname: '/Components/teacherForm/teacherFormUpdate', 
                          query: {id: data[0]._id},
                        }} className="bg-green-600 hover:bg-green-700 text-white font-semibold py-[5px] px-[5px] ml-3 transition rounded-full duration-200 ease-in-out transform text-sm">
                    ...
                    <Modal open={open} onClose={() => setOpen(false)}>
                      <div className="text-center w-56">
                        ...
                        <div className="flex gap-4">
                          <button onClick={() => deleteHandler(data[0]._id)} className="btn bg-red-600 hover:bg-red-700 rounded-md w-full">Delete</button>
                          <button
